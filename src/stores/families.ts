...
export const useFamiliesStore = defineStore('families', () => {
  ...
  async function fetchFamilies(page: number = 1) {
    loading.value = true;
    error.value = null;
    try {
      const result = await familiesApi.getFamilies(page, pageSize.value);
  ...
  return { families, loading, error, currentPage, pageSize, total, totalPages, isLoading, hasError, fetchFamilies, createFamily, updateFamily, deleteFamily, reset };
});

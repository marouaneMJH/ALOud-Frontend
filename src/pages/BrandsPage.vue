<template>
  <CrudLayout page-title="Brands Management">
    <div class="space-y-6">
      <!-- Create/Edit Modal -->
      <Modal v-if="showModal" @close="closeModal" :title="editingBrand ? 'Edit Brand' : 'Create New Brand'">
        <form @submit.prevent="submitBrand" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Brand Name</label>
            <input
      ...
      <Modal v-if="showDeleteModal" @close="closeDeleteModal" title="Delete Brand">
        <div class="space-y-4">
          <p class="text-gray-300">Are you sure you want to delete this brand? This action cannot be undone.</p>
          <div class="flex gap-4 pt-4">
            <button
              @click="confirmDelete"
              :disabled="isDeleting"

<template>
  <CrudLayout page-title="Occasions Management">
    <div class="space-y-6">
      <Modal v-if="showModal" @close="closeModal" :title="editingItem ? 'Edit Occasion' : 'Create New Occasion'">
        <form @submit.prevent="submitForm" class="space-y-4 max-h-96 overflow-y-auto">
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Occasion Name</label>
            <input v-model="formData.name" type="text" placeholder="e.g., Evening" required class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Description</label>
            <textarea v-model="formData.description" placeholder="Occasion description..." rows="2" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Recommended Sillage</label>
            <input v-model="formData.recommendedSillage" placeholder="e.g., Light to Medium" type="text" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Recommended Longevity</label>
            <input v-model="formData.recommendedLongevity" placeholder="e.g., 6-8 hours" type="text" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
      ...
      <Modal v-if="showDeleteModal" @close="closeDeleteModal" title="Confirm Deletion">
        <div class="space-y-4">
          <p class="text-gray-300">Are you sure you want to delete this occasion?</p>
          <div class="flex gap-4 pt-4">
      ...
      <DataTable :items="items" :columns="columns" entity-name="Occasion" :is-loading="isLoading" :has-error="hasError" :error="error" @create="openCreateModal" @edit="openEditModal" @delete="startDelete" />
    </div>
  </CrudLayout>
</template>
...
const formData = ref<CreateOccasionDto>({ name: '', description: '', recommendedSillage: '', recommendedLongevity: '' });
const columns = [
...
const openCreateModal = () => {
  ...
  formData.value = { name: '', description: '', recommendedSillage: '', recommendedLongevity: '' };
  formError.value = '';
...
const openEditModal = (item: OccasionDto) => {
  ...
  formData.value = { name: item.name, description: item.description, recommendedSillage: item.recommendedSillage || '', recommendedLongevity: item.recommendedLongevity || '' };
  formError.value = '';
  showModal.value = true;
};
...
const submitForm = async () => {
  ...
  try {
    if (editingItem.value) {
      await occasionsApi.updateOccasion(editingItem.value.id, { id: editingItem.value.id, ...formData.value });
    } else {

<template>
  <CrudLayout page-title="Accords Management">
    <div class="space-y-6">
      <Modal v-if="showModal" @close="closeModal" :title="editingItem ? 'Edit Accord' : 'Create New Accord'">
        <form @submit.prevent="submitForm" class="space-y-4 max-h-96 overflow-y-auto">
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Accord Name</label>
            <input v-model="formData.name" type="text" placeholder="e.g., Warm" required class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Description</label>
            <textarea v-model="formData.description" placeholder="Accord description..." rows="2" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Effect</label>
            <input v-model="formData.effect" placeholder="e.g., Fresh" type="text" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Popularity (0-100)</label>
            <input v-model.number="formData.popularity" type="number" min="0" max="100" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
      ...
      <Modal v-if="showDeleteModal" @close="closeDeleteModal" title="Confirm Deletion">
        <div class="space-y-4">
          <p class="text-gray-300">Are you sure you want to delete this accord?</p>
          <div class="flex gap-4 pt-4">
      ...
      <DataTable :items="items" :columns="columns" entity-name="Accord" :is-loading="isLoading" :has-error="hasError" :error="error" @create="openCreateModal" @edit="openEditModal" @delete="startDelete" />
    </div>
  </CrudLayout>
</template>
...
const formData = ref<CreateAccordDto>({ name: '', description: '', effect: '', popularity: 50 });
const columns = [
...
const openEditModal = (item: AccordDto) => {
  ...
  formData.value = { name: item.name, description: item.description, effect: item.effect || '', popularity: item.popularity || 50 };
  formError.value = '';
  showModal.value = true;
};
...
const submitForm = async () => {
  ...
  try {
    if (editingItem.value) {
      await accordsApi.updateAccord(editingItem.value.id, { id: editingItem.value.id, ...formData.value });
    } else {

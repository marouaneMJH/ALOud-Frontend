<template>
  <CrudLayout page-title="Tags Management">
    <div class="space-y-6">
      <Modal v-if="showModal" @close="closeModal" :title="editingItem ? 'Edit Tag' : 'Create New Tag'">
        <form @submit.prevent="submitForm" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Tag Name</label>
            <input v-model="formData.name" type="text" placeholder="e.g., Bestseller" required class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gold-300 mb-2">Description</label>
            <textarea v-model="formData.description" placeholder="Tag description..." rows="3" class="w-full px-4 py-2 border border-gold-700 bg-gray-900 text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
          </div>
      ...
      <Modal v-if="showDeleteModal" @close="closeDeleteModal" title="Confirm Deletion">
        <div class="space-y-4">
          <p class="text-gray-300">Are you sure you want to delete this tag?</p>
          <div class="flex gap-4 pt-4">
      ...
      <DataTable :items="items" :columns="columns" entity-name="Tag" :is-loading="isLoading" :has-error="hasError" :error="error" @create="openCreateModal" @edit="openEditModal" @delete="startDelete" />
    </div>
  </CrudLayout>
</template>
...
const submitForm = async () => {
  ...
  try {
    if (editingItem.value) {
      await tagsApi.updateTag(editingItem.value.id, { id: editingItem.value.id, ...formData.value });
    } else {
